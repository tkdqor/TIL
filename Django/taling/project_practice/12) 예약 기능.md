...
**2) 우리 서비스는 굳이 1번처럼 하지 않아도 된다. 그리고 2번 방법은 우리가 굳이 배치 프로그램을 짜지 않아도 이러한 비슷한 패턴을 우리가 찾아낼 수 있다면 다른 사례에서도 적용할 수 있다. restaurant.py에서 함수를 설정해서 해보도록 하자.**
