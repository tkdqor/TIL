...
class PayView(TemplateView):                     # 코드 수정한 내역 반영
    ...
    def get_context_data(self, status):
        ...
        # 이 때, 토스에서 결제 성공 라디이렉트 URL에 포함되서 돌아오는 3가지 쿼리 파라미터를 이렇게 정의하는 것
...
- PayView는 위의 과정에서 3번에 해당한다. 그래서 "결제 승인 API 호출하기" 부분을 해줘야 한다. 그래서 해당 부분을 보면 header를 넣게 되어있다. 우리가 요청을 보낼 때 Authorization이라는 header가 존재하는데, 이건 인증 키를 싣을 때 많이 사용하는 header이다. 그래서 세션 토큰이라든가 인증 키라든가 API 키라든가 이런 값들을 많이 넣는 header이다. 여기에 정해진 Basic .. 이런 값을 넣으라고 하고 있다.
