...
- **다음과 같은 오류가 발생한다. 우리가 settings.py로 가서 볼 수 있는 STATICFILES_DIRS = [ BASE_DIR / 'static', ] --> 이 부분에서 BASE_DIR이 루트 디렉터리를 의미한다. 그래서 이 디렉터리 내부에 static이라는 디렉터리가 등장해야 하는데 지금은 없기 때문에 위와 같은 오류가 발생한 것이다.** 
