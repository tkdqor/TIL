...
- 위의 코드에서 쿼리가 어떻게 나가는건지 보면, lazy-loading 방식이라고 볼 수 있다. 실제로 runserver를 가동하면 http://localhost:8000/ 이 메인 페이지에 추천식당이라고 뜬다. 일단 지금은 아무것도 없으니까 DBeaever에서 recommendations 모델에 데이터를 추가하자.   
...
- 다시 위의 template 코드를 보면, recommendations는 바로 recommendation이라는 모델이다. --> 여기서 restaurant 필드로 접근하면, ForeignKey로 이것과 연결된 Restaurant 모델을 불러오게 된다. 
  ...
  - 지금이야 히트를 치는 횟수가 5번밖에 되지 않지만, 사실 저 자세히 보면 Category 모델도 있기 때문에 모델 개수만큼 또 치게 된다. 그리고 이미지는 개별적으로 있으니까 정확하게는 --> 2N + X + 1번 히트를 치게 된다. 이렇게 많은 회수롤 데이터베이스에 히트를 치게 되면, 당연히 데이터베이스에 부하가 많이 걸리게 된다.
