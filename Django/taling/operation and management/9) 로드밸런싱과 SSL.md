...
### SSL 인증서 연결하기
- 이렇게 S3까지 준비된 EC2 서버를 여러 대를 띄워서 로드 밸런서를 앞에 붙이고 SSL 인증서를 연결해서 HTTPS, 즉 TLS 통신을 지원해보자. 
- 우리가 만든 docker 이미지를 AWS에서 사용할 수 있게 만든 것을 AMI라고 부르기도 한다. AWS에 미리 올려둔 AMI을 가지고 서버 2대를 띄워보기.
- 강사님이 만든 AMI는 시즌1 배포를 다룰 때 배운 내용대로 nginx와 함께 인스턴스를 구성한 것처럼 만들어놓았다. 그래서 일단 인스턴스를 띄우기만 해도 80번 포트로 접속이 가능해진다. 
