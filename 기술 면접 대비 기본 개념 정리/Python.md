...
- C나 C++ 같은 오래된 언어에서는 이러한 메모리 관리를 코드로 직접 작성해야 한다. 그런데 잘못 작성하게 되면 메모리 공간이 꽉 차서 컴퓨터가 뻗게된다. 그래서 자바나 파이썬을 비롯한 새로운 언어에서는 **가비지 컬렉터라는 것을 도입하기 시작했다. 즉, 프로그래머가 일일이 하던 메모리 관리를 대신해주는 것으로 사용되지 않는 객체들을 정리해 주면서 메모리 공간을 여유 있게 만들어주는 것이다. 즉, 가비지 컬렉터는 특정 때에 특정 방식으로 필요없는 정보들, garbage들을 쓸어담아다 버려준다.**
