...
- **2) B-Tree**
  - 보통 균형있는 이진탐색트리는 O(log n)의 시간복잡도를 가지고, 균형없는 이진탐색트리는 O(n)을 가질 수 있다. **이렇게 비효율적으로 이진탐색 트리를 사용하는 걸 개선해서 나온게 B-Tree 자료구조이다.**
...
### 인덱스의 종류
- 데이터베이스에서 클러스터링이란, 실제 데이터와 무리를 이룬다는 의미이고 / 논-클러스터링이란, 실제 데이터와 무리를 이루지 않는다는 의미이다.
  - 그래서 클러스터링 인덱스는 실제 데이터와 같은 무리의 인덱스를 의미한다. (ex. 실제 데이터가 정렬된 사전) 그래서 클러스터링 인덱스는 실제 그 인덱스가 가리키는 것이 데이터의 페이지 주소가 된다.
  - 논-클러스터링 인덱스는 실제 데이터와 다른 무리의 별도의 인덱스를 의미한다. (ex. 실제 데이터 탐색에 도움을 주는 별도의 찾아보기 페이지) 그래서 논-클러스터링 인덱스는 그 인덱스가 가리키는 것이 별도의 주소이다.
