...
## 컴파일러란
- 특정 프로그래밍 언어로 쓰여있는 문서를 다른 프로그래밍 언어 혹은 컴퓨터 언어로 옮기는 일종의 번역 프로그램. 그래서 프로그래밍 언어를 사용해 컴파일러에게 명령을 전달하면 컴파일러가 그 명령을 0과 1로 변환해 컴퓨터에게 전달하게 된다.
- [관련 블로그](https://yanacoding.tistory.com/56)

<br>

## 컴퓨터 구성 요소
- CPU : 컴퓨터의 머리 역할. CPU는 따로 데이터를 저장하지 않기 때문에 데이터를 연산하거나 처리하려면 데이터를 CPU로 보내줘야 한다.
- 메모리 : CPU의 개인 작업 공간. CPU가 보조기억장치에게 필요한 데이터를 요청하지 않아도, 작업이 필요한 큰 데이터들을 보조기억장치에서 메모리로 한 번 옮겨놓고 메모리안에서 작업하면 된다.
- 보조기억장치 : 대표적으로 하드디스크가 있다. 컴퓨터의 창고 역할을 하며 전원이 꺼져도 이 안에 데이터는 남아있다. 

- ex) **포토샵 프로그램을 실행시킨다고 해보자.** 포토샵 아이콘을 더블클릭하면 포토샵 실행 파일의 바로가기를 클릭한 것이 되고 "보조기억장치"에 저장된 프로그램을 실행하게 되는 것이다. 이 때, 바로 켜지지 않고 로딩이 걸리는데 보조기억장치에서 '실행에 필요한 데이터'가 '메모리'로 올라가고 있는 것이다. 그래서 이 과정이 완료되면, 포토샵 프로그램에서 어떤 걸 수행하든 빠르게 할 수 있는데, CPU가 메모리 위에서 작업을 빠르게 하기 때문이다.
  - 이러한 하드웨어를 **운영체제**가 관리하기 때문에 우리가 편리하게 프로그램을 사용할 수 있게 된다.

<br>

## 네트워크
- **LAN(Local Area Network)** : 컴퓨터가 연결된 작은 지역을 의미. 학교 컴퓨터실 하나, 아파트 하나, 커피숍 하나하나가 모두 LAN이다. 이 LAN을 연결하는 선을 LAN선이라고 부른다.
- **MAN(Metropolitan Area Network)** : 도시의 여러 LAN을 하나로 연결해서 MAN을 구성한다.
- **WAN(Wide Area Network)** : 도시와 도시, 나라와 나라를 모두 연결해서 WAN을 구성한다. 
- 옛날에는 이 선들들 연결하기 위해 수많은 공사가 진행되었으나, 이제는 이 신호를 와이파이, 4G, 5G 등 무선으로 만들어 데이터를 주고받을 수 있게 되었다.
